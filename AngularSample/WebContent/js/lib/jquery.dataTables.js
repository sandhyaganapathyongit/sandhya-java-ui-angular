...
//This variable Maintains newly added row Index on search
var addRowIndex = 0;
var addSearch= false;
var ctrlA = false;
var ctrlC = false;
	...
	$.fn.dataTable = function( oInit )
	{
                ...
                ($.inArray('asc', oCol.asSorting) == -1 && $.inArray('desc', oCol.asSorting) == -1) )
      {
		...
		function _fnFeatureHtmlFilter ( oSettings )
		{
			...
			nFilter.innerHTML = '<label style=\"font-size:12px;\">'+sSearchStr+'</label>';
			...
			jqFilter.bind( 'keydown.DT', function (e) {

				if(e.which == 17) isCtrl = true;
				if(e.which == 65) isKeyA = true;
				if(e.which == 67)isKeyC =true;
			...
			jqFilter.bind( 'keyup.DT', function(e) {
				...
				if(e.which == 65)isKeyA = false;
				if(e.which == 67)isKeyC =false;
//				if(!isKeyA && !isCtrl)
//				{
			...
			else
			{
					   ...
					   sInput.indexOf(oSettings.oPreviousSearch.sSearch) !== 0 )
				{
					...
					for ( i=0 ; i<oSettings.aiDisplayMaster.length ; i++ )
					{
						...
						//MyInstall change starts-arc8599
						//Below change is done to display newly added row when search is done
						//This pushes the value of newly added Row by default.
						var n=_fnGetTdNodes ( oSettings, i );
						var y= n[0];
						var avalue =$(y).html();
						//span tag is used in UI to differentiate newly added row. 
						if(avalue.toLowerCase().indexOf("<span name=\"abcmyxzinstall\"></span>")!=-1)
						{
							//addRowIndex gives position of newly added row in displayed List based on search.
							addRowIndex = searchIndex;
							//This Flag is set to true in case AddNewRow is present.
							addSearch = true;
						}
						
						
						if ( (avalue.toLowerCase().indexOf("<span name=\"abcmyxzinstall\"></span>")!=-1) || rpSearch.test(oSettings.asDataSearch[i]) )
						{
			  ...
			  else
				{
				 var addRowIndx2 = 0;
			  	...
			  	for ( i=0 ; i<oSettings.asDataSearch.length ; i++ )
					{
			  		//MyInstall change starts-arc8599
			  		//addRowIndex is checked to display newly added row.
			  		if(addSearch == true)//Incase of AddNew Row present this loop executes.
			  		{
				  		if ( addRowIndex != i & ! rpSearch.test(oSettings.asDataSearch[i]) )
							{
				  			oSettings.aiDisplay.splice( i-iIndexCorrector, 1 );
				  			iIndexCorrector++;
				  		}
			  		}else//MyInstall change ends
			  		{
			  		...
			  		//This maintains position of newly added row.
			  		//Length is taken as New Row is always added at the bottom.
			  		addRowIndx2 = oSettings.aiDisplay.length - 1;
			  	}
			  	addRowIndex = addRowIndx2;
			  }
			}
				...
				(oSettings.aaSorting.length !== 0 || oSettings.aaSortingFixed !== null) )
			{
				...
				oSettings.aiDisplayMaster.sort( function ( a, b ) {
					...
					for ( i=0 ; i<iSortLen ; i++ )
					{
						...
						var avalue = _fnGetCellData( oSettings, a, iDataSort, 'sort');
						var bvalue = _fnGetCellData( oSettings, b, iDataSort, 'sort');
						if(sDataType!='numeric' && (avalue.indexOf("<span name=\"abcmyxzinstall\"></span>")!=-1 ||bvalue.indexOf("<span name=\"abcmyxzinstall\"></span>")!=-1)){
							iTest = 0;
							return iTest;
						}else{
